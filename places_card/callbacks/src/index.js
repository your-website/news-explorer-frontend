// В данном примере коллбэк-функции передаются в специальные открытые для записи поля классов.
  ...
  .then(([cardsInfo, userInfo]) => {
    ...
    cardListSection.setData(cardsInfo.map(item => getCardData(item)),(info) => {
      ...
      return [].concat(info).reduce((fragment, cardData) => {
        ...
        card.removeCallback((card) => {
          api.deleteCard(card.id)
            .then(() => {
              cardsInfo = cardsInfo.filter(item => {
                return item._id !== cardData._id;
              });
              card.remove()
            })
        });
        ...
        card.openCallback(() => {
          picturePopup.open(cardData.link);
          return picturePopup
        });

        card.likeCallback((card) => {
